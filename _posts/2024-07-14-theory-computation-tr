...
\
![](https://lh7-us.googleusercontent.com/docsz/AD_4nXcEmI8KCyiTlj_RuDBve6hrE0Li9oUfaoyDjiPtZzO_NwQDVpHCgf5ncHIwyE6PNNQy4ag-5EHiDMTNf6c86Pr1yaMM5aiTXowBKqdNzLeMjpF0p8xVUsH8ADaI0rvVHHenbZazvqqc0e1_DqgejOG_3Lk?key=gsd_dwRg7Ob3d3qc7sHmeA)
...
Two rooms/endpoints (_A_ and _B_) are connected by a tunnel. In each room, there is a lever (let it be called _a_ in _A_ and called _b_ in _B_) that starts in a False state, and there is a closed door (let the door in _A_ be called _x_ and the door in _B_ be called _y_) that leads to the corridor between _A_ and _B_. Setting lever _a_ to True opens _x_ for exactly enough time such that Lara can only run into the corridor and cannot return to _A_; setting lever _b_ to True does the same for _y_ and room _B_.

In the corridor, there is a button _c_, which, if pressed, will open door _y_ if lever _a_ is also set to True, and will open door _x_ if lever _b_ is also set to True. When either door is successfully opened using button _c_, then levers _a_ and _b_ are reset to False, and Lara can run into the non-origin room, completing the traversal. The door will only be open for enough time for Lara to enter the room but not return to the corridor. To go back to the original room after a traversal, Lara will have to set the lever in the endpoint room to True to open the closest door. Then, she’d have to hit the button _c_ to open the door she first passed through and return to the starting room. Remember, we are assuming Lara is playing optimally to reach the end goal and will not make any moves that will permanently lock in the tunnel. So, Lara would not hit button _c_ to open either _x_ and _y_ and then simply choose to stay in the middle corridor because it would permanently keep her stuck there since the outer levers will be set to False as a result of hitting the button. Also, the doors are timed in such a way that if Lara chooses to pass over them, either from one of the outer rooms into the corridor or vice versa, she would only have enough time to just get past the door and not return to the room she started from. And while she could move around one of the outer rooms and not pass through the door, this wouldn’t make Lara better off as it doesn’t get her closer to the end. 

The levers _a_ and _b_ cannot ever both be set to True at the same time because then Lara would have to be able to traverse the tunnel without having either _A_ or _B_ be set to False. This is impossible because crossing from one room to the other requires the original lever used to exit the first room to be set to False. If Lara spawns in room _J_ (where _J_ is either _A_ or _B_), then _j_ (where _j_ is the lever of room _J_) must be reset to False once the door to _J’_ (where if _J_ is _A_, then _J’_ is _B_, and if _J_ is _B_, then _J’_ is _A_) is opened, which is necessary to set _j’_ to True. Therefore, both levers can never be set to True at the same time.
...
[**_Indy Hat_**](https://www.youtube.com/watch?v=2ui85HwTX1s) **_Property_**

_Timed doors that abide by this property force Lara to either stay in her current room or move into the room that the doorway had just opened. These doors are positioned/timed in such a way that Lara cannot return to the original room she exited via the door before it shuts. If the doorway is too far and/or there isn’t enough time, she could never make it through, and if it is too close, she could step into the other room and then return to the original room. The exact timing/distance would require placing the door at a location with a timer such that when running full speed using the shortest path from the switch/button to and through the doorway that was just opened would mean it shuts exactly behind Lara as she enters the room, preventing her from returning._

By having all of the doors included in the tunnel abide by this property in respect to the button/lever that are directly adjacent to it, one of which must be _c_, then all of the doors begin to function as one-way doors, and in the context of the tunnel, force the player to complete the traversal before being able to return to the original starting point. 
...
8. Swim through water-filled level to reach lever to remove water (16:33-16:51)
